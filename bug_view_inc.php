...
$t_target_version_string   = $t_show_target_version ? prepare_version_string( $t_bug->project_id, version_get_id( $t_bug->target_version, $t_bug->project_id ) ) : '';
...
$t_os_version = $t_show_os ? string_display_line( $t_bug->os_build ) : '';
...
if(config_get('view_issue_button_notes') | config_get('view_issue_button_sendmail') | config_get('view_issue_button_history') |  config_get('view_issue_button_next')){
	...
	if(config_get('view_issue_button_notes')){
		print_link_button( '#bugnotes', lang_get( 'jump_to_bugnotes' ) );
	}

	# Send Bug Reminder
	if(config_get('view_issue_button_sendmail')){
		if( $t_show_reminder_link ) {
			print_link_button( $t_bug_reminder_link, lang_get( 'bug_reminder' ) );
		}

		if( !is_blank( $t_wiki_link ) ) {
			print_link_button( $t_wiki_link, lang_get( 'wiki' ) );
		}
				...
				if( is_array( $t_hook ) ) {
					foreach( $t_hook as $t_label => $t_href ) {
						if( is_numeric( $t_label ) ) {
							...
							print_link_button( $t_href, $t_label );
						}
					}
				} elseif( !empty( $t_hook ) ) {
	...
	if(config_get('view_issue_button_history')){
		if( !is_blank( $t_history_link ) ) {
			# History
			print_link_button( $t_history_link, lang_get( 'bug_history' ) );
		}
	}
		...
		if( $t_bugslist ) {
			...
			if( false !== $t_index ) {
				if( isset( $t_bugslist[$t_index-1] ) ) {
					print_link_button( 'view.php?id='.$t_bugslist[$t_index-1], '&lt;&lt;' );
				}
...
echo '<td class="bug-description" colspan="6">', string_display_links( $t_bug->description ), '</td>';
echo '</tr>';
...
echo '<tr>';
	...
	# date submitted
	echo '<th class="bug-date-submitted category" width="15%">', lang_get( 'date_submitted' ), '</th>';
	echo '<td class="bug-date-submitted" width=100%>', date( config_get( 'normal_date_format' ), $t_bug->date_submitted ), '</td>';
echo '</tr>';
...
echo '<tr>';
	...
	# date updated
	echo '<th class="bug-last-modified category">', lang_get( 'last_update' ),'</th>';
	echo '<td class="bug-last-modified">', date( config_get( 'normal_date_format' ), $t_bug->last_updated ), '</td>';
echo '</tr>';
...
echo '<tr>';
	...
	echo '<td class="bug-severity">', string_display_line( get_enum_element( 'severity', $t_bug->severity ) ), '</td>';
